...
The entire process can be done with 1 named object and 2 pipes.
```{r}
...
The <b class = 'fxn'>dplyr package</b> is does a lot of the heavy lifting for data wrangling. The functions I use most often are:
<ul>
...
<li><b class = 'fxn'>mutate</b>: Creates a new column.</li>
...
<li><b class = 'fxn'>summarize</b>: goes hand in hand with `group_by` to then summarize by the grouping columns, like calculating the average temperature for each site, year, and month with daily temperature data.</li>
</ul>

Other packages I frequently use are:
<ul>
<li><b class = 'fxn'>tidyr</b>: for reshaping datasets from long to wide and vise versa. </li>
...
<p class = 'fxn'>Average min, max, mean temperature by month</p>
```{r}
...
temp2 <- temp |> group_by(SiteCode, year, mon) |> 
